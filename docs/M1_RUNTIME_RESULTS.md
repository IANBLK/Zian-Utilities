...
`RUNTIME_TEST_MATRIX.md` defines `YOUER-01` as the core command subset, `YOUER-02` as natural/Fishing/Poké Snack parity, and `YOUER-03` as Zian GTS coexistence with Generation Control. `M1_RUNTIME_PROTOCOL.md` requires NeoForge acceptance before formal Youer acceptance. The mapping below organizes existing evidence without overriding that ordering.

- `YOUER-01 — observed, formal acceptance pending`: the accepted Youer observations include enable/disable/active/status/list and live mutation. The supplied Youer `latest.log` records a successful Gen2 disable at 04:09:00 and `active` commands at 04:09:04 and 04:09:58; `latest (1).log` and `latest (2).log` record further `active` commands with two players. Server logs do not include the chat responses. The operator also observed that naturally appearing Pokemon matched the currently active generation. Existing command results can be mapped when the NeoForge baseline is accepted; do not infer unlogged responses from command issuance alone.
- `YOUER-02 — functional behavior observed, formal acceptance pending`: the operator observed natural spawns restricted to the active generation in separate areas, including Gen2-only and Gen7-only states. The already accepted Fishing and Poké Snack Youer observations include both mechanisms operating together under generation filtering. This supports the three required paths; formal parity still depends on the NeoForge gate and preserved case references, not an automatic replay of the same gameplay.
- `YOUER-03 — coexistence observed, transaction proof pending`: the existing evidence records Zian GTS runtime initialization and clean shutdown on Youer, and prior operator observation that a GTS receive did not interfere with Generation Control. The supplied logs do not identify a specific receive transaction, species or active-generation response at that transaction time. Therefore startup alone cannot close the GTS interaction case. Recover the prior transaction record if available; request one focused receive test only if no verifiable record survives after NeoForge acceptance.
