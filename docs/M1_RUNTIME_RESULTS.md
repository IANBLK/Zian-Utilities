...
| YOUER-03 | Functional coexistence observed: an existing Pokémon and a Zian GTS receive were previously exercised without Generation Control interference. | The supplied logs show GTS startup but no identifiable receive transaction. Recover the earlier transaction record, or perform one focused receive only if that record is unavailable. |

## Youer compatibility mapping against the runtime matrix

`RUNTIME_TEST_MATRIX.md` defines `YOUER-01` as the core command subset, `YOUER-02` as natural/Fishing/Poké Snack parity, and `YOUER-03` as Zian GTS coexistence with Generation Control. Youer is the intended deployment target. The revised `M1_RUNTIME_PROTOCOL.md` evaluates its deployment readiness from Youer evidence while retaining a separate NeoForge-first gate for a broader cross-loader M1 release-candidate claim.
...
- `YOUER-03 — functional coexistence observed; transaction proof pending`: the existing evidence records Zian GTS runtime initialization and clean shutdown on Youer, and prior operator observation that a GTS receive did not interfere with Generation Control. The supplied logs do not identify a specific receive transaction, species or active-generation response at that transaction time. Therefore startup alone cannot close the GTS interaction case. Recover the prior transaction record if available; request one focused receive test only if no verifiable record survives.

## Remaining acceptance work

For the intended Youer server deployment, `PERSIST-03`, `PERF-01`, `PERF-02`, DIAG-01 and DIAG-02 have direct accepted Youer evidence. `YOUER-02` has accepted functional evidence. Preserve existing command-response evidence for `YOUER-01` if available, and recover the prior specific GTS receive record for `YOUER-03`; if no transaction record survives, perform only one focused receive check. Do not repeat the direct Kill or broad spawn/Fishing/Poké Snack tests.
